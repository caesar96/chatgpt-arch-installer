...
The launcher uses X11/XWayland by default by passing
`--ozone-platform=x11`. It also reads the optional
`${XDG_CONFIG_HOME:-$HOME/.config}/chatgpt-flags.conf` file and passes every
non-empty, non-comment line to ChatGPT as one command-line flag. The file is
used by normal CLI launches, desktop launches, and automatic relaunches after
updates or decoration changes. The default path is
`~/.config/chatgpt-flags.conf`; when `XDG_CONFIG_HOME` is set, it is located at
`$XDG_CONFIG_HOME/chatgpt-flags.conf`. The installer does not create this file.

The only persistent application rendering flag currently used by this project
is `--ozone-platform=x11`. `--no-patches` is a wrapper control, the user-data
directory flag is generated by the launcher, and `ELECTRON_FORCE_WINDOW_MENU_BAR`
is an environment variable rather than a flags-file entry. The
`--disable-gpu` flag is not used by default and prevented your bundled build
from creating a window.

Create the file manually to use native Wayland instead:

```bash
mkdir -p ~/.config
printf '%s\n' \
  '# Use native Wayland instead of the default X11/XWayland path.' \
  '--ozone-platform=wayland' > ~/.config/chatgpt-flags.conf
```

Add additional flags as separate lines and restart ChatGPT after changing the
file. The `global-menu` patch remains intended for X11/XWayland sessions.
...
```bash
chatgpt --no-patches --ozone-platform=x11
```

This is a diagnostic launch only; X11 is also the installer's default. The
`--disable-gpu` flag is not enabled by default and may prevent this bundled
build from creating a window. The splash and the main login page are separate
startup stages, so a successful splash does not prove that the main renderer
can paint correctly. If this command opens the login screen, the live image's
graphics or Wayland/XWayland setup is the likely cause. Otherwise, run
`chatgpt --debug` and inspect `<installation>/state/patch-diagnostic.log`.
...
~/.config/chatgpt-flags.conf               Optional Electron command-line flags
